  ...
  useEffect(() => {
    const fetchUserVouchers = async () => {
      ...
      try {
        ...
        const formattedVouchersForModal = (fetchedVouchersFromApi || []).map(apiVoucher => ({
          id: apiVoucher.voucherId,
          title: `Giảm ${formatPrice(apiVoucher.discountAmount || 0)}`,
          condition: (apiVoucher.minOrderAmount != null && apiVoucher.minOrderAmount > 0)
            ? `Đơn hàng từ ${formatPrice(apiVoucher.minOrderAmount)}`
            : "Áp dụng cho mọi đơn hàng",
          code: apiVoucher.code,
          expiry: apiVoucher.expiryDate
            ? new Date(apiVoucher.expiryDate).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' })
            : "Vô thời hạn",
        }));
    ...
    <div className="w-full min-h-screen flex flex-col bg-gray-50 text-gray-800">
      ...
      <AnimatePresence>
        {isCartOpen && ( // Or isCartPanelOpen if using context
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-black/50" onClick={closeCart} aria-hidden="true" /> // Or closeCartPanel
        )}
      </AnimatePresence>
             ...
             <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ duration: 0.3, ease: "easeOut" }}
               ...
               <div className="h-full flex flex-col">
                 <div className="p-6 border-b border-gray-200 flex justify-between items-center sticky top-0 bg-white z-10">
                   <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2"> <FaShoppingCartIcon size={20} /> Giỏ hàng ({totalItemsCount})</h2>
                   <button onClick={closeCart} className="text-gray-500 hover:text-gray-700 p-2 rounded-md hover:bg-gray-100 transition-colors" aria-label="Close cart"> <FaTimes size={20} /> </button> {/* Or closeCartPanel */}
                 </div>
                   ...
                   {cartItems.length === 0 ? (
                     <p className="text-gray-500 text-center py-10">Your cart is empty</p>
                       ...
                       <div className="space-y-4">
                           {cartItems.map((item) => (
                             <div key={item.id} className="flex border-b border-gray-200 pb-4 last:border-b-0 last:pb-0" >
                               ...
                               <div className="ml-4 flex-grow flex flex-col justify-between">
                                 <div> <h4 className="font-semibold text-gray-800 text-base mb-1">{item.name}</h4> </div>
                           ...
                           {isLoadingVouchers ? (
                               <p className="text-gray-500 text-center py-2">Đang tải mã ưu đãi...</p>
                           ) : voucherError ? (
                               <p className="text-red-500 text-sm text-center py-2">Lỗi: {voucherError}</p>
                           ) : apiVouchers.length > 0 ? (
                              <div className="pt-4 border-t border-gray-200 text-center">
                                   <h3 className="font-semibold text-gray-800 mb-4">MÃ ƯU ĐÃI DÀNH CHO BẠN</h3>
                                   <button onClick={openVoucherModalFromLayout} className="text-blue-600 hover:underline font-semibold text-sm"> Xem tất cả mã ưu đãi ({apiVouchers.length}) </button>
                              </div>
                           ) : (
                               !isLoadingVouchers && !voucherError && <p className="text-gray-500 text-center py-2 text-sm">Không có mã ưu đãi nào.</p>
                           )}
                       </> )}
                  </div>
                 <div className="p-6 border-t border-gray-300 bg-white sticky bottom-0 z-10">
                   ...
                   {cartItems.length > 0 && (
                         <div className="flex justify-center items-center mt-4 text-sm font-semibold">
                             <Link to="/cart" onClick={closeCart} className="text-blue-600 hover:underline"> Xem giỏ hàng </Link> {/* Or closeCartPanel */}
                         </div>
                    )}
                 </div>
               </div>
